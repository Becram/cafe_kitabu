...
class WPBookList_Front_End_Library_UI {
	...
	public function __construct($which_table, $searchType = null, $searchTerm = null, $sort = null) {
		...
		if($this->table == $wpdb->prefix.'wpbooklist_jre_saved_book_log'){
			...
			$temp = array_pop($temp);
			$this->display_options_table = $wpdb->prefix.'wpbooklist_jre_settings_'.$temp;
		}
	...
	private function output_sort_by(){
		$string1 = '<div id="wpbooklist-select-sort-div">
                        <select id="wpbooklist-sort-select-box">    
                            ...
                            <option value="year-read">Year Finished</option>
	...
	private function output_search(){
		$string1 = '<div id="wpbooklist-search-div">
                    	<div id="wpbooklist-search-checkboxes">
	                        <p>Search By:
	                            <input id="wpbooklist-book-title-search" type="checkbox" name="book-title-search" value="book-title-search">Title</input>
	                            <input id="wpbooklist-author-search" type="checkbox" name="author-search" value="author-search">Author</input>
	                            <input id="wpbooklist-cat-search" type="checkbox" name="cat-search" value="author-search">Category</input>
	                        </p>
                    	</div>
                    	<div>
                    		<input id="wpbooklist-search-text" type="text" name="search-query" value="Search...">
                    	</div>
	                    <div id="wpbooklist-search-submit">
	                        <input disabled data-table="'.$this->table.'" id="wpbooklist-search-sub-button" type="button" name="search-button" value="Search"></input>
	                    </div>
                	</div>';
	...
	private function output_stats(){
		...
		<div class="wpbooklist_stats_tdiv">
         	<p class="wpbooklist_control_panel_stat">Total Books: '.number_format($this->total_book_count).'</p>
            <p class="wpbooklist_control_panel_stat">Finished: '.number_format($this->total_book_read_count).'</p>
            <p class="wpbooklist_control_panel_stat">Signed: '.number_format($this->total_book_signed_count).'</p>
            <p class="wpbooklist_control_panel_stat">First Editions: '.number_format($this->total_book_first_edition_count).'</p>
            <p class="wpbooklist_control_panel_stat">Total Pages Read: '.number_format($this->total_pages_read_count).'</p>
            <p class="wpbooklist_control_panel_stat">Categories: '.number_format($this->total_category_count).'</p>
            <p class="wpbooklist_control_panel_stat">Library Completion: ';
				...
				if($onpage_key <= $this->display_options_actual->booksonpage){
					$string2 = $string2.'<div class="wpbooklist_entry_div">
		                ...
		                <div class="wpbooklist_inner_main_display_div">
		                    ...
		                    if($this->display_options_actual->hiderating != 1 && $this->display_options_actual->hiderating != null && $book->rating != 0 && $book->rating != null){
...
/*
			if($remainder > 0 && $i == (1-$whole_pages)){
				$string1 = $string1.'<div data-library="'.$this->table.'" data-page="'.$i.'" data-per-page="'.$this->display_options_actual->booksonpage.'" class="wpbooklist-pagination-page-div" id="wpbooklist-pagination-page-'.$i.'"> '.((($i+1)*$this->display_options_actual->booksonpage)-($this->display_options_actual->booksonpage-1)).'-'.$this->total_book_count.' </div>';
			}
			*/
		}
