...
class WPBookList_Book {
	...
	public function __construct($action = null, $book_array = null, $id = null) {
		...
		if(($action == 'add' || $action == 'edit') && $book_array != null){
	...
	private function gather_amazon_data(){
    	...
    	if(function_exists('file_get_contents')){
    		...
    		error_log('file_get_contents Amazon status: '.$status);
    		if($status == 200){
    			error_log('file_get_contents Amazon equals check');
    	...
    	if($result == ''){
    		if (function_exists('curl_init')){ 
				...
				if($this->options_results->amazonapisecret != null && $this->options_results->amazonapisecret != '' && $this->options_results->amazonapipublic != null && $this->options_results->amazonapipublic != ''){
					$data = array('api_public'=>$this->options_results->amazonapipublic, 'api_secret'=>$this->options_results->amazonapisecret, 'book_title' => $this->title, 'associate_tag' => $this->options_results->amazonaff, 'isbn' => $this->isbn);
				} else {
				...
				error_log('cUrl Amazon status: '.$responsecode);
				if($responsecode == 200){
					error_log('cUrl Amazon equals check');
				}
				curl_close($ch);
	    	} else {
	...
	private function gather_google_data(){
    	...
    	if(function_exists('file_get_contents')){
    		...
    		list($version, $status, $text) = explode(' ', $http_response_header[0], 3);
    		error_log('Google File Get Contents Status: '.$status);
    		if($status == 200){
    			error_log('file_get_contents Google equals check');
    	...
    	if($result == ''){
    		if (function_exists('curl_init')){ 
				...
				$result = curl_exec($ch);
				$responsecode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
				error_log('cUrl Google status: '.$responsecode);
				if($responsecode == 200){
					error_log('cUrl Google equals check');
				}
				curl_close($ch);
	    	} else {
	        	//#TODO: Log an error here in a log class like maintainme's saying both file_get_contents and cURL aren't available.
	...
	private function gather_open_library_data(){
    	...
    	if(function_exists('file_get_contents')){
    		...
    		list($version, $status, $text) = explode(' ', $http_response_header[0], 3);
    		error_log('file_get_contents OpenLibrary Status: '.$status);
    		if($status == 200){
    			error_log('file_get_contents OpenLibrary equals check');
    	...
    	if($result == ''){
    		if (function_exists('curl_init')){ 
				...
				curl_setopt($ch, CURLOPT_URL, $url);
				$responsecode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
				error_log('cUrl OpenLibrary status: '.$responsecode);
				if($responsecode == 200){
					error_log('cUrl OpenLibrary equals check');
				}
				curl_close($ch);
	    	} else {
	        	//#TODO: Log an error here in a log class like maintainme's saying both file_get_contents and cURL aren't available.
	...
	private function gather_itunes_data(){
    	...
    	if(function_exists('file_get_contents')){
    		...
    		list($version, $status, $text) = explode(' ', $http_response_header[0], 3);
    		error_log('file_get_contents iTunes status: '.$status);
    		if($status == 200){
    			error_log('file_get_contents iTunes equals check');
    	...
    	if($result == ''){
    		if (function_exists('curl_init')){ 
				...
				curl_setopt($ch, CURLOPT_URL, $url);
				$responsecode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
				error_log('cUrl iTunes status: '.$responsecode);
				if($responsecode == 200){
					error_log('cUrl iTunes equals check');
				}
				curl_close($ch);
	    	} else {
	        	//#TODO: Log an error here in a log class like maintainme's saying both file_get_contents and cURL aren't available.
	...
	private function edit_book(){
	    ...
	    $format = array( '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'); 
	...
	public function delete_book($library, $book_id, $delete_string){
		...
		$wpdb->query($wpdb->prepare( "ALTER TABLE $library MODIFY ID BIGINT(255) NOT NULL", $library));
		...
		$wpdb->query($wpdb->prepare( "ALTER TABLE $library MODIFY ID BIGINT(255) AUTO_INCREMENT", $library));
