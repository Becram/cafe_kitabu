...
class WPBookList_Backup_Settings_Form {
	...
	public static function output_backup_settings_form(){
		...
		$string2 = '<div id="wpbooklist-backup-select-library-label" for="wpbooklist-backup-select-library">Select a Library to Backup:</div>
            ...
            $string5 = '<div id="wpbooklist-backup-select-library-label">Select a Library to Restore:</div>
            ...
            $string9 = '';
            foreach(glob(LIBRARY_DB_BACKUPS_UPLOAD_DIR.'*.*') as $filename){
                $filename = basename($filename);
                $display_name = explode('_-_', $filename);
                $string9 = $string9.'<option class="wpbooklist-backup-actual-option" data-table="'.$display_name[0].'" id="'.$filename.'" value="'.$filename.'">'.$display_name[1].' - '.date('h:i a', $display_name[2]).'</option>';
            }
		...
		$string11 = ' <div id="wpbooklist-addbackup-success-div"></div></div>';

		echo $string1.$string2.$string3.$string4.$string5.$string6.$string7.$string8.$string9.$string10.$string11;
